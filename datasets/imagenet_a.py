...
class ImageNetA(DatasetBase):
    """
    ImageNet-A (natural adversarial) 评测集
    ...
    def _read_imagenet_style_folder(self, folder_root, wnid_to_label_1000, classnames_list):
        """
        读取 ImageNet-A 文件夹，返回 Datum 列表和 mask
        """
        if not os.path.exists(folder_root):
            raise RuntimeError(f"ImageNet-A folder not found: {folder_root}")
        ...
        print(f"加载 test (ImageNet-A) 数据集，共 {valid_classes} 个类别（1000类子集）...")
